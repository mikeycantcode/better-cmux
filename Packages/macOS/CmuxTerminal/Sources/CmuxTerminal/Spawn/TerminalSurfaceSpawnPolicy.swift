...
public struct TerminalSurfaceSpawnPolicy: Sendable {
    ...
    /// The environment key under which the agent capability brief is exported
    /// (`CMUX_AGENT_CAPABILITY_BRIEF`), or `nil` when the brief is disabled.
    ///
    /// The brief text lives in the app target, so the app computes both this
    /// key and ``agentCapabilityBriefText`` and passes them through the policy;
    /// the package only folds them into the spawned environment. Both must be
    /// non-`nil` for the brief to be exported.
    public var agentCapabilityBriefEnvironmentKey: String?

    /// The agent capability brief text exported under
    /// ``agentCapabilityBriefEnvironmentKey``, or `nil` when the brief is
    /// disabled.
    ///
    /// Computed by the app target (the brief's source of truth) because the
    /// package cannot import it. Both this and
    /// ``agentCapabilityBriefEnvironmentKey`` must be non-`nil` for the brief
    /// to be exported.
    public var agentCapabilityBriefText: String?
    ...
    public init(
        ...
        agentCapabilityBriefEnvironmentKey: String? = nil,
        agentCapabilityBriefText: String? = nil
    ) {
        ...
        self.agentCapabilityBriefEnvironmentKey = agentCapabilityBriefEnvironmentKey
        self.agentCapabilityBriefText = agentCapabilityBriefText
    }
}
