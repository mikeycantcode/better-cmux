...
final class CmuxSettingsFileStore {
    ...
    private func parseAppSection(
        ...
        if let raw = jsonString(section["codeEditor"]) {
            if let choice = CodeEditorChoice(rawValue: raw) {
                snapshot.managedUserDefaults[CodeEditorPreferenceSettings.key] = .string(choice.rawValue)
            } else {
                logInvalid("app.codeEditor", sourcePath: sourcePath)
            }
        }
    ...
    private func parseAutomationSection(
        ...
        if let value = jsonBool(section["manualEditDiff"]) {
            snapshot.managedUserDefaults[ManualEditDiffSettings.manualEditDiffKey] = .bool(value)
        }
